...
const matchesConditions = (
  ...
  for (const context of contexts) {
    if (contextTypes.has(context as ContextMenuType)) {
      inContext = true;
      break;
    }
  }
  ...
  if (props.targetUrlPatterns && props.targetUrlPatterns.length > 0 && targetUrl) {
    if (!props.targetUrlPatterns.some((pattern: string) => matchesPattern(pattern, targetUrl))) {
      return false;
    }
  }
...
export class ContextMenusAPI {
  ...
  private buildMenuItemsFromTemplate = (menuItemTemplates: ContextItemConstructorOptions[]) => {
    ...
    for (const item of menuItemTemplates) {
      ...
      if (item.props.parentId !== undefined && item.props.parentId !== null) {
        const parentMenuItem = itemMap.get(String(item.props.parentId));
        if (parentMenuItem) {
  ...
  private onClicked(
    ...
    const data: chrome.contextMenus.OnClickData = {
      ...
      mediaType: params?.mediaType as "image" | "audio" | "video" | undefined, // Corrected type assertion
